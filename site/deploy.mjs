...
const FILES = ["index.html", "icon.png", "BrightnessSteps-setup.exe"];
...
// --- verify the page and the download are actually reachable -----------------
const page = await fetch(LIVE_URL + "?nocache=" + Date.now());
console.log(`\nverify: index    → HTTP ${page.status}`);
if (!page.ok) process.exit(1);

const dl = await fetch(LIVE_URL + "BrightnessSteps-setup.exe", { method: "HEAD" });
const size = Number(dl.headers.get("content-length") || 0);
console.log(`verify: download → HTTP ${dl.status} (${(size / 1024 / 1024).toFixed(2)} MB)`);
if (!dl.ok || size < 500_000) { console.error("installer did not upload intact"); process.exit(1); }
